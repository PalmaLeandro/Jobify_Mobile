...
public class SkillsEditionPresenter extends BasePresenter<SkillsEditionFragment>{
    ...
    public void addSkill(final String skill) {
        if(UserAuthService.getInstance().getUserProfile().getSkills().contains(skill)){
            Toast.makeText(getView().getContext(),R.string.skill_already_added_string,Toast.LENGTH_LONG).show();
            return;
        }
        if (skill.length()>0){
           ...
           Toast.makeText(getView().getContext(), R.string.provide_complete_skill_message,Toast.LENGTH_LONG).show();
        }
    }
